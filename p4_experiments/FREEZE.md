...
- **Provenance** on every Phase-8d HHL exploratory instance record (121 files): `sweep_provenance` block carries `qdk_versions` (qiskit / qiskit-aer / qsharp pinned versions), deterministic `seed` (`0x50414d50`), `ts_utc` of run, `hardware_profile`, `pre_registered` boolean, `canonical_phase8d_track` flag, `appendix_only` flag, and `selection_manifest_path` pointing to the Phase-8d selection manifest. `git_commit` is the sentinel string `"unknown"` because per-record git capture was not wired into the run; the run-time tarball is the integrity reference (`canonical/release/zenodo_bundle.tar.gz` + `.sha256`). Aggregate manifests under `canonical/outputs/` carry `generated_utc`. `profile_sha` and `preregistration_tag` are not separate fields; the per-instance `hardware_profile` field plus `pre_registered` flag plus `canonical/PRE_REGISTRATION.md` together cover the same role.
- **Cosmetic figure regeneration 2026-05-12.** `canonical/outputs/figures/h_figures_sidecar.json` and `canonical/outputs/figures/p4_hoefler_crossover.json` (with their PNG/PDF outputs) were re-rendered on 2026-05-12 for visual presentation tweaks; data, source inputs, and numerical claims are unchanged. See [`canonical/DECISIONS_LOG.md`](canonical/DECISIONS_LOG.md) 2026-05-12 entry.
