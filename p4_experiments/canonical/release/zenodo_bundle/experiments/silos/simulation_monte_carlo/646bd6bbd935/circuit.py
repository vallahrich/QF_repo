"""SM10 — Accelerated quantum Monte Carlo with mitigated error on noisy quantum computer.
...
@register(
    ...
    description="SM10 — generic variational ansatz stretch (Accelerated quantum Monte Carlo with mitigated error on noisy quantum computer)",
)
...
def _full_oracle(instance: Dict[str, Any], bare: QuantumCircuit) -> QuantumCircuit:
    ...
    n = int(p["n_qubits"])
    a = _ansatz_only(p)
    qc = QuantumCircuit(n, n, name="SM10_full")
