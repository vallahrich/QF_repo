"""SM2 — Quantum spectral method for stochastic processes (paper 8f553bfb1077)."""
...
@register(
    ...
    description="Spectral fBM state prep (n=5, L=3) + canonical QAE",
)
def build_bare(instance: Dict[str, Any]) -> QuantumCircuit:
    ...
    qc = QuantumCircuit(sp.num_qubits, sp.num_qubits, name="SM2_bare")
...
def _full_oracle(instance: Dict[str, Any], bare: QuantumCircuit) -> QuantumCircuit:
    ...
    return canonical_qae(state_prep=sp, num_eval_qubits=int(p["num_eval_qubits"]), name="SM2_full")
...
register_accounting(
    ...
    oracle=_full_oracle,
    oracle_variant="spectral-prep+canonical-QAE",
    notes="Full = spectral state prep + canonical QAE. Bare = spectral state prep only.",
)
