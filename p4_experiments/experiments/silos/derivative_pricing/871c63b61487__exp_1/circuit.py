"""SD8 — Extracting a function encoded in amplitudes of a quantum state by tensor network and orthogonal function expansion.
...
@register(
    label="SD8",
    paper_id="871c63b61487",
    silo="derivative-pricing",
    algorithm_family="other-gate-based",
    description="SD8 — generic variational ansatz stretch (Extracting a function encoded in amplitudes of a quantum state by tensor network and orthogonal function expansion)",
)
...
def _full_oracle(instance: Dict[str, Any], bare: QuantumCircuit) -> QuantumCircuit:
    ...
    n = int(p["n_qubits"])
    a = _ansatz_only(p)
    qc = QuantumCircuit(n, n, name="SD8_full")
