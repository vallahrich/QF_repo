"""SD10 — A variational quantum algorithm for the Feynman-Kac formula.
...
@register(
    label="SD10",
    paper_id="9b277c9234e8",
    silo="derivative-pricing",
    algorithm_family="other-gate-based",
    description="SD10 — generic variational ansatz stretch (A variational quantum algorithm for the Feynman-Kac formula)",
)
...
def _full_oracle(instance: Dict[str, Any], bare: QuantumCircuit) -> QuantumCircuit:
    ...
    n = int(p["n_qubits"])
    a = _ansatz_only(p)
    qc = QuantumCircuit(n, n, name="SD10_full")
